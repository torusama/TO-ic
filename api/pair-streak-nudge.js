const { sendPairStreakNudge, verifyFirebaseRequest } = require("./_lib/mail-automation.js");
...
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    if (status >= 500) {
      console.error("Pair streak nudge failed:", error);
    }
    response.status(status).json({ ok: false, error: error.message || "Could not send pair streak reminder." });
  }
};
